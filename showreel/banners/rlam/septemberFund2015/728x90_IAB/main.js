(function(){
	...
	function runSequencer(){
				...
				**/
				...
				case 4:
					if(readyToRun(8.5))
					{
						...
						showElement(gilbert);
						TweenLite.to(gilbert, 1, {alpha:1});
						TweenLite.from(get("screen_2"), 1, {alpha:0, scaleY:0, y:-32, onComplete:function(){showElement(get("p0"))}});
						TweenLite.to(get("p0"), .5, {alpha:1, delay:1});
						TweenLite.to(get("disclaimerLink1"), .2, {css:{display:'none'}, delay:.8});
						TweenLite.to(get("disclaimerLink2"), .2, {alpha:1, delay:.8});
						showElement(get("p1"));
						showElement(get("p2"));
		...
		// TweenLite.to(gilbert, 0, {scaleX:.8, scaleY:.8, x:63, y:85});
		...
		// 	TweenLite.to(magGlass, 1, {scaleX:2.05, scaleY:2.05, x:233, y:29, ease:Power1.easeInOut, delay:1.5});
		// 	TweenLite.to(incomeText, 1, {scaleX:1.9, scaleY:1.9, x:-92, y:5, ease:Power1.easeInOut, delay:1.5});
		// }

		// function runGilbert(){
		// 	showElement(gilbert);
		// 	TweenLite.to(gilbert, .5, {alpha:1});
		// 	// TweenLite.to(gilbert, .5, {x:-28, y:83, scaleX:.75, scaleY:.75, delay:7}); // ENABLE WHEN TESTING
	    ...
	    // TESTING CALLS
	    // var  delay = setTimeout(runMagAnimation, 750);
	    // var  delay = setTimeout(runGilbert, 750);
	}
