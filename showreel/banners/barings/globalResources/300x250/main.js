				...
				**/
		        case 0:
		            if (readyToRun(0.6)) {
		                ...
		                // hat DROPS
		                TweenLite.from(objs[7], .5, { y:-160, scaleX:2.5, scaleY:2.5, ease:Power1.easeIn, ease:Power1.easeIn });
		        ...
		        case 1:
		            if (readyToRun(1.3)) {
		                ...
		                // glasses DROP
		                TweenLite.from(objs[5], .6, { y:-50, scaleX:2.5, scaleY:2.5, ease:Power1.easeIn });
		                TweenLite.from(objs[5], .1, { alpha:0 });

		                // clipboard
		                TweenLite.from(objs[3], 1.3, { x:-10, y:70, delay:.2 });
		                TweenLite.from(objs[3], .1, { alpha:0, delay:.2 });

		                // pencil
		                TweenLite.from(objs[8], .75, { y:60, ease:Power1.easeOut, delay:.8 });
		                TweenLite.from(objs[8], .1, { alpha:0, delay:.8 });

		                // gps DROP
		                TweenLite.from(objs[6], .5, { x:100, y:10, scaleX:2.5, scaleY:2.5, ease:Power1.easeIn, ease:Power1.easeIn, delay:.5 });
		        ...
		        case 2:
		            if (readyToRun(2)) {
		                ...
		                // bins DROP
		                TweenLite.from(objs[2], 1, { x:-5, y:60, scaleX:2.5, scaleY:2.5, ease:Power1.easeIn, delay:.2});
		                TweenLite.from(objs[2], .1, { alpha:0, delay:.2 });

		                // beanbag
		                TweenLite.from(objs[10], .6, { y:-30, ease:Power1.easeIn, delay:.1});
		                TweenLite.from(objs[10], .1, { alpha:0, delay:.1 });

		                // beans
		                TweenLite.from(objs[1], .3, { y:-60, delay:.7});
		        ...
		        case 3:
		            if (readyToRun(2.9)) {
		                ...
		                // coal DROP
		                TweenLite.from(objs[4], .6, { x:-86, y:-10, rotation:-20, scaleX:2.5, scaleY:2.5, ease:Power1.easeIn });
