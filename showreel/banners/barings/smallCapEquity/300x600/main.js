				...
				**/
		        ...
		        case 1:
		            if (readyToRun(1)) {
		                // design paper
		                showElement(objs[13]);
		                TweenLite.from(objs[13], 1, { y: 280 });
		                TweenLite.from(objs[13], .1, { alpha: 0 });


		                // mask
		                showElement(objs[6]);
		                TweenLite.from(objs[6], 1.2, { scaleX: 2.5, scaleY: 2.5, x: 100, y: -415, rotation: 10, ease: Power1.easeIn });	// DROPS
		                TweenLite.from(objs[6], .1, { alpha: 0 });

		                // keys
		                showElement(objs[5]);
		                TweenLite.from(objs[5], .9, { scaleX: 2.5, scaleY: 2.5, x: 25, y: 280, rotation: -30, delay: .1, ease: Power1.easeIn });	// DROPS
		                TweenLite.from(objs[5], .1, { alpha: 0, delay: .1 });

		                // tape
		                showElement(objs[14]);
		                TweenLite.from(objs[14], .8, { x: -90, y: 60, rotation: 75 });
		        ...
		        case 2:
		            if (readyToRun(2)) {
		                ...
		                // cork
		                showElement(objs[15]);
		                TweenLite.from(objs[15], .8, { scaleX: 2.5, scaleY: 2.5, y: -170, rotation: 30, delay: .3, ease: Power1.easeIn });	// DROPS
		        ...
		        case 5:
		            if (readyToRun(3.2)) {
		                ...
		                //    scaleX: 3, scaleY: 3, x: 5, y: 170, delay: .1, ease: Power1.easeIn
		                //});	// DROPS
		                //TweenLite.from(objs[2], .1, { alpha: 0, delay: .3 });
		                // coinHK
		                showElement(objs[12]);
		                TweenLite.from(objs[12], .5, { scaleX: 3, scaleY: 3, x: 5, y: 170, delay: .1, ease: Power1.easeIn });
