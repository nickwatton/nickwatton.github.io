				...
				**/
		        ...
		        case 1:
		            if (readyToRun(1)) {
			        	//compass
			            showElement(objs[0]);
		                TweenLite.from(objs[0], .9, { scaleX: 2.5, scaleY: 2.5, y: -320, rotation: 30, ease: Power1.easeIn });	// DROPS
			            ...
			            // teacup
		                showElement(objs[2]);
		                TweenLite.from(objs[2], .5, { x: 100, delay: 1, });
		                TweenLite.from(objs[2], .1, { alpha: 0 });


			            // football
		                showElement(objs[3]);
		                TweenLite.from(objs[3], .5, { x: -150, delay: 1.2, });
		                TweenLite.from(objs[3], .1, { alpha: 0 });

			        	//glasses
			            showElement(objs[4]);
		                TweenLite.from(objs[4], .9, { scaleX: 2.5, scaleY: 2.5, y: -360, rotation: 30, delay: 1.5, ease: Power1.easeIn });	// DROPS
		        ...
		        case 2:
		            if (readyToRun(2)) {
		                ...
		                // journal
		                showElement(objs[5]);
		                TweenLite.from(objs[5], .5, { y: 150, });
		                TweenLite.from(objs[5], .1, { alpha: 0 });

		                // phone
					    showElement(objs[6]);
					    TweenLite.from(objs[6], .8, { scaleX: 2, scaleY: 2.5, y: -600, rotation: 20,  ease: Power1.easeIn });
		        ...
		        case 3:
		            if (readyToRun(2.4)) {
		            						   ...
		            						   // sim
					    showElement(objs[7]);
					    TweenLite.from(objs[7], .8, { scaleX: 2.5, scaleY: 2.5, y: -400, rotation: 20, ease: Power1.easeIn });
		                ...
		                //cookie
			            showElement(objs[9]);
		                TweenLite.from(objs[9], .7, { scaleX: 2.5, scaleY: 2.5, y: -320, rotation: 20,  ease: Power1.easeIn });	// DROPS
		        ...
		        case 5:
		            if (readyToRun(3.2)) {
						...
						// chopstick 1
		                showElement(objs[12]);
		                TweenLite.from(objs[12], .5, { scaleX: 1.5, scaleY: 1.5, x: 120, y: -205, rotation: -30});	// DROPS
		                TweenLite.from(objs[12], .1, { alpha: 0 });

		                // chopstick 2
		                showElement(objs[13]);
		                TweenLite.from(objs[13], .5, { scaleX: 2.5, scaleY: 1.5, x: -120, y: -205, rotation: 30,  });	// DROPS
