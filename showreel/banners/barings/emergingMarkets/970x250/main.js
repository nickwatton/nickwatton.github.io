				...
				**/
				...
				case 1:
					if (readyToRun(1)) {
						...
						// ipad
						showElement(objs[1]);
						TweenLite.from(objs[1], .5, { x: -100, delay: .5, });
						TweenLite.from(objs[1], .1, { alpha: 0 });

						// teacup
						showElement(objs[2]);
						TweenLite.from(objs[2], .5, { x: 100, delay: .9, });
				...
				case 2:
					if(readyToRun(2))
					{
						//glasses
						showElement(objs[4]);
						TweenLite.from(objs[4], .9, { scaleX: 2.5, scaleY: 2.5, y: -360, rotation: 30, delay: .3, ease: Power1.easeIn });	// DROPS
						TweenLite.from(objs[4], .1, { alpha: 0 });

						// journal
						showElement(objs[5]);
						TweenLite.from(objs[5], .5, { y: 160, delay: .5, });
				...
				case 3:
					if(readyToRun(2.5))
					{
						// phone
						showElement(objs[6]);
						TweenLite.from(objs[6], .8, { scaleX: 2.5, scaleY: 2.5, y: -400, rotation: 20, ease: Power1.easeIn, delay: .8 });
						TweenLite.from(objs[6], .1, { alpha: 0, delay: .8 });

						// sim
						showElement(objs[7]);
						TweenLite.from(objs[7], .8, { scaleX: 2.5, scaleY: 2.5, y: -400, rotation: 20, delay: 1 , ease: Power1.easeIn});
