(function() {
    ...
    gradFillCols_Sky = ["#1c2445","#1e315b","#193c6c","#0b548d","#196da4","#448ebd","#66abad"], gradFillRatios_Sky = [0,.29,.41,.50,.56,.64,1],
    maskStart = 75,
    matteVO = {
        // matte
        mW:648,
    ...
    function setUpMask() {
        ...
        matte.graphics.f("red").mt(0,0).lt(matteVO.mW,0).lt(matteVO.iW,matteVO.iH).lt(0,matteVO.mH).lt(0,0).cp(); // Using tinyAPI http://goo.gl/oMK7vL
    ...
    function animationSequencer(){
        switch(createjs.Ticker.getTicks())
        {
            ...
            case 8:
                ...
                TweenLite.to(paperPlane, 2, {y:paperVO.planeStartY-7, ease:easeInOutQuad, override:false});
    ...
    function update() {
        if (running) {
            ...
            clouds.x = (clouds.x < -395) ? 100 : clouds.x - cloudVO.cloud1Step;
            clouds2.x = (clouds2.x < -395) ? 100 : clouds2.x - cloudVO.cloud2Step;
    ...
    function moveMaskOverImage(pos) {
        container.x = pos;
        paperContainer.x = -container.x-1;
